...
def kubechecker(host):
    ...
    url7 = "https://" + host + ":" + str(port3) + "/runningpods"
    
    try:
        response6 = requests.get(url6, verify=False, timeout=1)
        response7 = requests.get(url7, verify=False, timeout=1)
        ...
        if (response7.status_code == 200 and ('namespace' in response7.text or 'Namespace' in response7.text) and ('pod' in response7.text or 'Pod' in response7.text)):
            print("+"*40)
            print("\033[91mKubernetes node allowing system:anonymous viewing of pods found: %s\033[0m" % url7)
            outfile.write("Kubernetes node allowing system:anonymous viewing of pods found:\n")
            outfile.write(url7)
            outfile.write("\n")
            unauthexeclist.append(url7)
        elif response7.status_code == 401 or response7.status_code == 403:
            print("+"*40)
            print('\033[33mKubernetes node returned "unauthorized" response: %s\033[0m' % url7)
            outfile.write('Kubernetes node returned "unauthorized" response:\n')
            outfile.write(url7)
            outfile.write("\n")
            authexec.append(url7)
        else:
            pass
